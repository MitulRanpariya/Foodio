...
const NavbarComponent = () => {
     ...
     // Filter meals for search dropdown
     ...
     // Handle click on a meal in dropdown
     const handleMealClick = (meal) => {
        setSearchTerm("");
        setShowDropdown(false);
        navigate(`/meals-by-meal-type/${meal.mealType.id}#meal-${meal.id}`);
     };
  ...
  return (
     <div className='header'>
          ...
          {/* Global Search Bar */}
          <div style={{ position: 'relative', maxWidth: 350, margin: '0 24px', flex: 1 }} ref={searchInputRef}>
            <input
              type="text"
              className="form-control"
              style={{ borderRadius: 30, border: '2px solid #1976d2', fontSize: 16, padding: '10px 20px', boxShadow: '0 2px 8px rgba(25, 118, 210, 0.10)' }}
              placeholder="Search meals..."
              value={searchTerm}
              onChange={e => { setSearchTerm(e.target.value); setShowDropdown(true); }}
              onFocus={() => setShowDropdown(true)}
            />
            {showDropdown && searchTerm && filteredMeals.length > 0 && (
              <div style={{ position: 'absolute', top: 44, left: 0, right: 0, background: '#fff', borderRadius: 18, boxShadow: '0 4px 16px rgba(25, 118, 210, 0.18)', zIndex: 1000, maxHeight: 320, overflowY: 'auto' }}>
                {filteredMeals.slice(0, 8).map(meal => (
                  <div
                    key={meal.id}
                    style={{ display: 'flex', alignItems: 'center', padding: '10px 16px', cursor: 'pointer', borderBottom: '1px solid #f0f4fa' }}
                    onClick={() => handleMealClick(meal)}
                  >
                    <img src={`data:image/png;base64,${meal.image}`} alt={meal.name} style={{ width: 36, height: 36, borderRadius: 8, objectFit: 'cover', marginRight: 12 }} />
                    <div style={{ flex: 1 }}>
                      <div style={{ fontWeight: 600, color: '#1976d2' }}>{meal.name}</div>
                      <div style={{ fontSize: 13, color: '#888' }}>{meal.mealType?.typeName}</div>
                    </div>
                    {localStorage.token && (
                      <button
                        className="btn btn-blue"
                        style={{ fontSize: 13, padding: '6px 14px', marginLeft: 8 }}
                        onClick={e => { e.stopPropagation(); /* addToCart logic here */ }}
                      >
                        Add to Cart
                      </button>
                    )}
                ...
                {filteredMeals.length > 8 && <div style={{ padding: 10, textAlign: 'center', color: '#1976d2' }}>More results...</div>}
              </div>
            )}
          </div>
